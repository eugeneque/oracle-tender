...
from app.models.analysis import Criticality, Requirement, TenderOutcome, WinPercentage
...
электроэнергии. Тебе даны сведения о закупке и уже посчитанные измерения AI-оценки.
Сформулируй итоговый блок для карточки.
...
- verdict: строго одно из: "go" (идти), "go_with_reservations" (идти с оговорками),
  "no_go" (не идти).
...
def _requirements_block(requirements: list[Requirement]) -> str:
    ...
    return "\n".join(
        f"{number}. [{requirement.criticality}] "
...
def _relevant_participations(
    ...
    Два признака родства, оба из формулировки раздела 5.5.1 ТЗ («похожие закупки у похожих
    заказчиков»):

    * закупка та же по смыслу — участие привязано к тендеру из списка похожих;
    * заказчик тот же — прошлый заход к этому же заказчику говорит о шансах не меньше, чем
      сходство текста ТЗ, и работает даже когда эмбеддинги ещё не посчитаны.

    Записи, попавшие по обоим признакам, не удваиваются.
    """
    ...
    customer = (tender.customer_name or "").strip()
    if len(customer) >= 4:
        by_customer = list(
            db.scalars(
                select(CompanyParticipation).where(
                    CompanyParticipation.manufacturer_id == mirtek.id,
                    CompanyParticipation.customer_name.is_not(None),
                    func.lower(CompanyParticipation.customer_name).like(
                        f"%{customer.lower()}%"
                    ),
                )
            )
        )
        fresh = [item for item in by_customer if item.id not in found]
        if fresh:
            reasons.append(f"по тому же заказчику — {len(fresh)}")
...
def _fallback_verdict(overall: Decimal | None) -> str:
    """Вердикт, когда модель вернула чушь или текстовый блок не удался.

    Границы те же, что у цветного бейджа списка (раздел 5.6 ТЗ): ≥80 — зелёный, 50-80 —
    жёлтый, ниже — красный. Иначе бейдж и вердикт в одной карточке противоречили бы друг
    другу.
    """
...
def compute_profile_score(
    ...
    verdict = _fallback_verdict(overall)
    ...
    try:
        ...
        if resume.verdict in {item.value for item in Verdict}:
...
def serialize(db: Session, score: AiProfileScore) -> dict:
    ...
    from app.models.ai_profile import SEVERITY_LABELS, VERDICT_LABELS
