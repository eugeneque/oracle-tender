"""Настройки подключения к Yandex AI Studio (раздел 5.4 ТЗ) — хранятся в БД и управляются из
`/settings` в интерфейсе (роль «Администратор»), а не в `.env`: заказчик не хочет заходить в
...
_UNCONFIGURED_MESSAGE = (
    "Подключение не настроено: заполните API-ключ и Folder ID в разделе «Интеграции» настроек."
)
