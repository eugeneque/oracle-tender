      ...
      <div className="rounded-xl border border-dashed border-white/10 p-6 text-center text-sm text-zinc-500">
        {noRequirements ? (
          <>
            ...
            <p className="mt-1.5">
              Из документации не извлечено ни одного требования, а расчёт сравнивает каталог
              продукции именно с ними. Сначала выполните «Анализ документов»; если он уже
              отработал и требований всё равно нет — вероятно, в документах закупки нет
              технического задания (так бывает у закупок на работы, где ТЗ лежит отдельным
              приложением).
            </p>
          </>
        ) : (
          <>
            ...
            <p className="mt-1.5">
              Запустите «Расчёт соответствия» — система сопоставит {requirementsCount}{" "}
              {pluralRequirements(requirementsCount)} с каталогом продукции и посчитает
              процент победителя (раздел 5.5 ТЗ).
            </p>
          </>
        )}
      </div>
